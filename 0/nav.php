<!-- ==================== NAV SECTION ======================== -->

<section class="bg_main nav" id="nav">
    <nav class="fg_center sub__nav">
        <div class="logo">
            <a href="./">
                <img src="img/logo.png" alt="Lynxs logo">
            </a>
        </div>
        <!-- (Down Syndrome)-NIPT -->
        <ul class="list_menu">
            <div class="close_menu" id='close_menu'><i class='bx bx-x' ></i></div>
            <li class="list_1">
                <a href="./" id="home" class="link__1">Home</a>
            </li>
            <li class="list_1">
                <a href="about-us" id="about" class="link__1">About Us</a>
            </li>
            <li class="list_1 mega_list" id="disable">
                
                <a class="link__1 ach" id="ach">DNA Services we offer</a>
                <ul class="list_2" id="li">
                    <li class="l_2">
                        <a href="services#paternity"  class="link__2">DNA Paternity Test</a>
                    </li>
                    <li class="l_2">
                        <a href="services#d_test"  class="link__2">Maternity DNA Test</a>
                    </li>
                    <li class="l_2">
                        <a href="services#immigration"  class="link__2">Immigration Test</a>
                    </li>
                    <li class="l_2">
                        <a href="services#sib_test"  class="link__2">Siblingship DNA Test</a>
                    </li>
                    <li class="l_2">
                        <a href="services#fam_test"  class="link__2">Family Reconstruction DNA Test</a>
                    </li>
                    <li class="l_2">
                        <a href="services#chrome_test"  class="link__2">Y-Chromosome Test: Male Lineage Testing</a>
                    </li>
                    <li class="l_2">
                        <a href="services#avun_test"  class="link__2">Avuncular DNA Test: Aunt-Uncle Testing</a>
                    </li>
                    <li class="l_2">
                        <a href="services#grandp_test"  class="link__2">Grandparent DNA Test</a>
                    </li>
                    <li class="l_2">
                        <a href="services#mt_test"  class="link__2">MtDNA TEST: Female Lineage Testing</a>
                    </li>
                    <li class="l_2">
                        <a href="services#bank_test" class="link__2">DNA Banking</a>
                    </li>
                    <li class="l_2">
                        <a href="services#twin_test"  class="link__2">Twin Zygosity Test</a>
                    </li>
                    <li class="l_2">
                        <a href="services#pt_test"  class="link__2">Non-invasive Prenatal Screeninig Test</a>
                    </li>
                    <li class="l_2">
                        <a href="services#p_test"  class="link__2">Non-invasive pre-paternity DNA test</a>
                    </li>
                    <li class="l_2">
                        <a href="services#c_test"  class="link__2">Cancer Risk Test</a>
                    </li> 
                </ul>
            </li>
            <li class="list_1">
                <a href="cancer-risk-test" id="cancer" class="link__1">Cancer Risk Test</a>
            </li>
            <li class="list_1">
                <a href="collection-centers" id="col_ct" class="link__1">Collection Centers</a>
            </li>
            <li class="list_1">
                <a href="blog/" id="blg" class="link__1">Blog</a>
            </li>
            <li class="list_1">
                <a href="contact" id="contact" class="link__1">Contact Us</a>
            </li>
        </ul>

        <div class="toggle">
            <i class='bx bx-menu box_icon' id="box_i"></i>
        </div>
    </nav>
</section>
<!-- ==================== END NAV SECTION ======================== -->